...
class LibraryManagement {
    ...
    // recursive approach coz its given in the function to do it recursively
    public BookNode findBook(BookNode currentBook, int bookId) {
        if(currentBook == null){ // if we reach end but haven't found return null book
            return null;
        }
        else if(currentBook.bookId==bookId) {
...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            // finding the line number and method name of exception origin directly
            System.out.println(e.getStackTrace()[0].getLineNumber() + "|" + e.getStackTrace()[0].getMethodName());
            e.printStackTrace();
        }
    }

    private static void closeWriter() throws IOException, NullPointerException {
        writer.flush();
    ...
    public static void _notIssued() {
        ...
        try {
            ...
            if(sortedUsers.isEmpty()){
                System.out.println("List of books not issued is Empty");
            }
            else {
    ...
    public static void _stockOut() {
        ...
        try {
            initializeWriter();
            if(sortedUsers.isEmpty()){
                System.out.println("No book in the Library is out of stock");
            }
            else {
            ...
            while ((line = br.readLine()) != null) { //here we read line by line from the input file which is promptsPS4.txt
                ...
                if(record[0].equalsIgnoreCase("checkOut") || record[0].equalsIgnoreCase("checkIn")){
                    if(record.length!=2) {
                        ...
                        // .trim() takes care of any strings that might have space in it before parsing to integer
                        _chkInChkOut(Integer.parseInt(record[1].trim()), record[0]);
                    }
                }
